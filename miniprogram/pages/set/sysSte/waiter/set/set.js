...
Page({
  ...
  data: {
    ...
    titel: {
      ...
      systemSet: ['申请和设置微信的收款账号信息', '灯控器的通讯设置', '打印参数及打印格式设置', '对与营业相关的参数进行调整', '添加桌台及缴纳年费查看年费截止日期','把桌台得器材与桌台码进行绑定', '设置门禁机的参数']
    }
  },
  ...
  async tap(e) {
    const path = this.data.setPosition + '.' + e.mark.obj + '.' + e.mark.index + '.' + e.mark.titel
    console.log(path)
    const values = this.data.power[this.data.setPosition][e.mark.obj][e.mark.index][e.mark.titel] === true ? false : true
    if (await this.changeDatabaseData(path, appData.shopInfo.shopFlag, values) === 'ok') {
      this.setData({
        ['power.' + this.data.setPosition + '.' + e.mark.obj + '[' + e.mark.index + '].' + e.mark.titel]: values
      })
      this.getKeys()
      wx.showToast({
        title: '保存成功!',
  ...
  async getData(shopFlag) {
    const power = await app.callFunction({
      name:'getPower',
      data:{
        shopFlag:shopFlag
      }
  ...
  getKeys() {
    ...
    for (let index = 0; index < this.data.power[this.data.setPosition].statement.length; index++) {
      const element = this.data.power[this.data.setPosition].statement[index];
      this.data.statementKeys.push(Object.keys(element)[0])
      this.data.statementValues.push(Object.values(element)[0])
    }
    for (let index = 0; index < this.data.power[this.data.setPosition].operate.length; index++) {
      const element = this.data.power[this.data.setPosition].operate[index];
      this.data.operateKeys.push(Object.keys(element)[0])
      this.data.operateValues.push(Object.values(element)[0])
    }
    for (let index = 0; index < this.data.power[this.data.setPosition].set.length; index++) {
      const element = this.data.power[this.data.setPosition].set[index];
      this.data.setKeys.push(Object.keys(element)[0])
      this.data.setValues.push(Object.values(element)[0])
    }
    for (let index = 0; index < this.data.power[this.data.setPosition].systemSet.length; index++) {
      const element = this.data.power[this.data.setPosition].systemSet[index];
  ...
  async onLoad(options) {
    ...
    this.setData({
      ...
      power: await this.getData(appData.shopInfo.shopFlag)
    })
    this.getKeys()
