...
Page({
  ...
  //下一页
  pageDown(e) {
    if (this.data.afterSaleIndex >= this.data.afterSaleArr.length - 1) {
      app.showModal('提示', '已经是最后一页了!')
      return
    }
    this.setData({
      afterSaleIndex: this.data.afterSaleIndex + 1
    })
  },
  //上一页
  pageUp(e) {
    if (this.data.afterSaleIndex < 1) {
      app.showModal('提示', '已经是第一页了!')
      return
    }
    this.setData({
      afterSaleIndex: this.data.afterSaleIndex - 1
  ...
  lookImage(e) {
    console.log(e)
    wx.previewImage({
      urls: this.data.afterSaleArr[this.data.afterSaleIndex].images,
  ...
  async refuseApply(e) {
    ...
    if (!refuseInfo.problemDescription) {
      app.showModal('提示', '请输入拒绝理由!')
      return
    }
    //修改服务器数据
    const res = await app.callFunction({
      ...
      data: {
        ...
        upData: {
          applyReturnStatus: this.data.orderInfo.applyReturnStatus === 1 ? 3 : 6,
          [`afterSaleReason${this.data.afterSaleIndex}.refuseInfo`]: refuseInfo
        }
      }
    })
    console.log(res)
    if (!res.success) {
      app.showModal('提示', '数据错误!请重新进入小程序后重试!')
      return
    }
    //修改本地数据
    this.data.orderInfo.applyReturnStatus = this.data.orderInfo.applyReturnStatus === 1 ? 3 : 6
    //数据传送至上一页
    const eventChannel = this.getOpenerEventChannel()
    eventChannel.emit('upData', this.data.orderInfo)
    wx.navigateBack()
  ...
  //确认收货退款
  async confirmRefund(e) {
    console.log(e)
    //计算退款金额
    const refundAmount = this.getLastAfterSalesAmount()
    const refundOrderNum = zx.createOrderNum(new Date(), 'RfMallOrder')
    const refund = {
      refund: {
        amount: refundAmount,
    ...
    console.log(res)
    if (!res.success) {
      app.showModal('提示', '退款失败!')
    }
    //退款
    const refoundRes = await zx.refund(app.callFunction, this.data.orderInfo.orderAmount / 100, refundAmount / 100, this.data.orderInfo.orderNum, refundOrderNum, this.data.orderInfo.sub_mch_id)
    console.log(refoundRes)
    if (refoundRes.data.result_code !== 'SUCCESS') {
      app.showModal('错误',refoundRes.data.err_code_des)
      return
    }
    app.showModal('提示', '退款成功,如未到账请联系客服!')
    //修改本地数据
    this.data.orderInfo.refund = {
      ...refund
    }
    this.data.orderInfo.applyReturnStatus = 0
    //数据传送至上一页
    const eventChannel = this.getOpenerEventChannel()
    eventChannel.emit('upData', this.data.orderInfo)
  },
  //获取最后一次申请退货的退款金额
  getLastAfterSalesAmount() {
    let amount = 0
    const order = this.data.orderInfo
    for (let index = 0; index < 100; index++) {
      if (`afterSaleReason${index}` in order) {
        amount = order[`afterSaleReason${index}`].refundAmount
      } else {
        break
      }
    }
    console.log('退款:' + amount / 100 + '元')
    return amount
  },
  //同意售后申请
  async agreeApply(e) {
    console.log(e)
    const afterType = this.data.afterSaleArr[this.data.afterSaleArr.length - 1].afterSaleType
    ...
    const res = await app.callFunction({
      ...
      data: {
        ...
        upData: {
          orderState: 4,
          applyReturnStatus: afterType === 1 ? 2 : 5 //2同意退货退款  5同意换货
        }
      }
    })
    if (!res.success) {
      app.showModal('提示', '上传数据失败!')
      return
    }
    //修改本地数据
    this.data.orderInfo.applyReturnStatus = afterType === 1 ? 2 : 5 //2同意退货退款  5同意换货
    this.data.orderInfo.orderState = 4
    //数据传送至上一页
    const eventChannel = this.getOpenerEventChannel()
    eventChannel.emit('upData', this.data.orderInfo)
    wx.navigateBack()
  },
  //获取此账单的所有售后记录 并默认显示第一个售后记录
  getAfterSaleArr() {
    const order = this.data.orderInfo
    const afterSaleArr = []
    for (let index = 0; index < 100; index++) {
      if (`afterSaleReason${index}` in order) {
        afterSaleArr.push(order[`afterSaleReason${index}`])
      } else {
        break
      }
    }
    this.setData({
      ...
      afterSaleIndex: afterSaleArr.length - 1
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
