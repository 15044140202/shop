...
function validateInput(event) {
  if (!event.order_id || !event.orderState || !event.expressData || !event.expressData.expressNum || event.expressData?.goodsList?.length < 1) {
...
exports.main = async (event, context) => {
  const { order_id, orderState, expressData ,operater} = event
  // 设置时区为亚洲/上海
  ...
  try {
    ...
    await transaction.collection('user_mall_order').where({
      ...
      data: {
        ...
        log: _.push(`订单发货--${getNowTime(new Date())}--物品数量:${expressData.goodsList.length}件--操作人:${operater}`)
      }
    })

    //添加物流信息
    await transaction.collection('user_mall_order_express').add({
      data:expressData
    })
    //提交事务
    ...
    return {
      success: true,
      message: `mall order transit  success`,
      data: { transactionId: transaction._id }
    }
  } catch (err) {
    // 回滚事务并返回错误信息
    await transaction.rollback();
    console.error('mall order transit error:', err); // 记录错误日志
    return {
      success: false,
      message: `mall order transit error`,
      data: err,
    }
  }
