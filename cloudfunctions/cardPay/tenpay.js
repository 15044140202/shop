...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox} = {}, debug = false) {
    if (!appid) throw {Error : 'appid fail'};
    if (!mchid) throw {Error : 'mchid fail'};
    if (!partnerKey) throw {Error : 'partnerKey fail'};
  ...
  async _parse(xml, type, signType) {
    ...
    switch (type) {
      case 'middleware_nativePay':
        break;
      default:
        if (json.return_code !== 'SUCCESS') throw {Error : json.return_msg || 'XMLDataError' ,data : json};
    }
    ...
    switch (type) {
      ...
      case 'getsignkey':
        break;
      default:
        if (json.result_code !== 'SUCCESS') throw {Error : json.err_code || 'XMLDataError' ,data : json}
    }
    ...
    switch (type) {
      case 'getsignkey':
        break;
      case 'middleware_refund': {
        if (json.appid !== this.appid) throw {Error:'appid不匹配'};
        if (json.mch_id !== this.mchid) throw {Error:'mch_id不匹配'}
      ...
      case 'transfers':
        if (json.mchid !== this.mchid) throw {Error:'mchid不匹配'}
        break;
      case 'sendredpack':
      case 'sendgroupredpack':
        if (json.wxappid !== this.appid) throw {Error:'appid不匹配'}
        if (json.mch_id !== this.mchid) throw {Error:'mchid不匹配'}
        break;
      case 'gethbinfo':
      ...
      case 'querybank':
        ...
        break;
      case 'combinedorder':
        if (json.combine_appid !== this.appid) throw {Error:'appid不匹配'}
        if (json.combine_mch_id !== this.mchid) throw {Error:'mch_id不匹配'}
        if (json.sign !== this._getSign(json, 'HMAC-SHA256')) throw {Error:'sign签名错误'}
        break;
      default:
        if (json.appid !== this.appid) throw {Error:'appid不匹配'}
        if (json.mch_id !== this.mchid) throw {Error:'mch_id不匹配'}
        if (json.sign !== this._getSign(json, json.sign_type || signType)) throw {Error:'sign签名错误'}
    }
    return json;
  }
  ...
  async _parseBill(xml, format = false) {
    if (util.checkXML(xml)) {
      let json = await util.parseXML(xml);
      throw  {Error:(json.err_code || json.return_msg || 'XMLDataError'),data:json};
    }
  ...
  async _request(params, type, cert = false) {
    ...
    if (status !== 200) throw {Error:'request fail'};
  ...
  middlewareForExpress(type = 'pay') {
    return async (req, res, next) => {
      ...
      try {
        if (typeof req.body !== 'string') throw { Error:'XMLDataError'};
  ...
  middleware(type = 'pay') {
    return async (ctx, next) => {
      ...
      try {
        if (typeof ctx.request.body !== 'string') throw {Error:'XMLDataError'};
  ...
  async payBank(params) {
    ...
    if (pub_key === '') throw  {Error:'get publickey fail'};
